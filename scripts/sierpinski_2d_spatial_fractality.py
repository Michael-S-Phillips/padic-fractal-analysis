#!/usr/bin/env python3
"""
2D Sierpinski Space Fractality Analysis

Compare geometric structure of elevation distributions in sierpinski space.
For each pixel, extract local window and compare the 2D spatial clustering
in sierpinski space to the global pattern.
"""
...
def main():
    ...
    print("=" * 70)
    print("2D SIERPINSKI SPATIAL FRACTALITY ANALYSIS")
    print("=" * 70)
    print(f"\nDEM shape: {dem_data.shape}")
    print(f"Total pixels: {len(dem_flat):,}")
    
    # Compute global sierpinski embedding
    print("\nComputing global sierpinski embedding...")
    global_x, global_y = embed_elevations_to_sierpinski(dem_flat, dem_flat)
    
    # Create global 2D histogram
    print("Creating global 2D histogram...")
    global_hist, xedges, yedges = compute_2d_histogram(global_x, global_y, bins=60)
    ...
    for y in range(h):
        ...
        for x in range(w):
            ...
            # Method 1: 2D Histogram Correlation
            ...
            similarity_map_hist[y, x] = max(0, corr)  # Clamp negative correlations to 0
...
def create_visualization(dem_data, sim_hist, sim_js, global_x, global_y, results_dir):
    ...
    stats_text = f"""
2D SIERPINSKI SPATIAL FRACTALITY

Histogram Correlation (Pearson r):
  Range: {valid_hist.min():.4f} to {valid_hist.max():.4f}
  Mean: {valid_hist.mean():.4f}
  Median: {np.median(valid_hist):.4f}
  Std: {valid_hist.std():.4f}

Jensen-Shannon Divergence:
  Range: {valid_js.min():.4f} to {valid_js.max():.4f}
  Mean: {valid_js.mean():.4f}
  Median: {np.median(valid_js):.4f}
  Std: {valid_js.std():.4f}

Method: 27×27 sliding windows
Local regions embedded in sierpinski
space, compared to global distribution
using 2D histogram correlation and JS
divergence (scale-invariant metrics).
    """
